...
void configureGate(uint8_t pin) {
  // Set the inactive level before enabling the output to prevent a startup flash.
  digitalWrite(pin, gateLevel(false));
  pinMode(pin, OUTPUT);
}

void writeGate(uint8_t pin, bool on) {
  const uint8_t level = gateLevel(on);
...
void updateFeedbackGate() {
  ...
  writeGate(HcdConfig::FEEDBACK_MOSFET_GATE_PIN,
